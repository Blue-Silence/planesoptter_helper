<template>
    <el-card shadow="hover">
      <!-- 大标题 -->
      <h2 class="airport-title">{{ props.airport.name }} ({{ props.airport.iata_code }} / {{ props.airport.icao_code }})</h2>
  
      <!-- 五颜六色的描述列表，展示机场的基础信息 -->
      <el-descriptions :column="2" border>
        <el-descriptions-item label="Name" label-class="desc-label">{{ props.airport.name }}</el-descriptions-item>
        <el-descriptions-item label="Code" label-class="desc-label">{{ props.airport.iata_code }} / {{ props.airport.icao_code }}</el-descriptions-item>
        <el-descriptions-item label="Location" label-class="desc-label">{{ props.airport.municipality }}, {{ props.airport.country.name }}</el-descriptions-item>
        <el-descriptions-item label="Continent" label-class="desc-label">{{ props.airport.continent }}</el-descriptions-item>
        <el-descriptions-item label="Scheduled Service" label-class="desc-label">{{ props.airport.scheduled_service }}</el-descriptions-item>
        <el-descriptions-item label="Elevation" label-class="desc-label">{{ props.airport.elevation_ft }} ft</el-descriptions-item>
        <el-descriptions-item label="Latitude" label-class="desc-label">{{ props.airport.latitude_deg }}</el-descriptions-item>
        <el-descriptions-item label="Longitude" label-class="desc-label">{{ props.airport.longitude_deg }}</el-descriptions-item>
        ...
        <el-descriptions-item label="Wikipedia" label-class="desc-label">
          <a :href="props.airport.wikipedia_link" target="_blank">{{ props.airport.wikipedia_link }}</a></el-descriptions-item>
      </el-descriptions>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-card class="runway-card" shadow="always">
            <h3>Runways</h3>
            <el-table :data="props.airport.runways" stripe border style="width: 100%" @row-click="openRunwayDialog" height=100%>
        ...
        <el-col :span="8">
          <el-card class="freq-card" shadow="always">
            <h3>Frequencies</h3>
            <el-table :data="props.airport.freqs" stripe border style="width: 100%" @row-click="openFreqDialog" height=100%>
        ...
        <el-col :span="8">
          <el-card class="navaid-card" shadow="always">
            <h3>Navaids</h3>
            <el-table :data="props.airport.navaids" stripe border style="width: 100%" @row-click="openNavaidDialog" height=100%>
